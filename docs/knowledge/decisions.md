...
## 2026-06-27 | AI-ассистент: guardrails + human-in-the-loop

AI никогда не пишет клиенту и не меняет стадию OPLACHENO. Все предложения AI создаются как `AiAction` со статусом PENDING — менеджер должен явно нажать «Принять» или «Отклонить». Только после APPROVED менеджер может копировать текст в мессенджер вручную. GuardrailsService фильтрует манипулятивные формулировки из предложений AI.

## 2026-06-27 | LifecycleStage: отдельный enum от PaymentStatus

Созданы разные enum-ы: `LifecycleStage` (на `VkConversation` — общий статус диалога/сделки) и `PaymentStatus` (на `Order` — статус платежа). Не смешивать — `LifecycleStage` описывает весь путь сделки, `PaymentStatus` — только финансовое состояние заказа.
